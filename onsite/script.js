/* ============================================================================
   NJTC PORTAL - ONSITE STAFF PORTAL SCRIPT
   New Jersey Tutoring Corps - Hub for Onsite Staff
   ============================================================================ */

// ── Auth helpers (global) ────────────────────────────────────────────────────

function njtcSwitchUser() {
  // Clears Pearl ID session only, reloads to show Pearl ID login again
  if (window.NJTCUserAuth) NJTCUserAuth.logout(); // clears njtc_user_v1
  localStorage.removeItem('NJTC_UID');
  location.reload();
}

function njtcFullSignOut() {
...
function switchTab(tabName) {
  document.querySelectorAll('.njtc-nav-tab').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.tab === tabName);
  });
  document.querySelectorAll('.njtc-tab-pane').forEach(pane => {
...
// ── Apply user profile to portal ─────────────────────────────────────────────

function applyUserProfile(user) {
    // Show the portal
    const portal = document.getElementById('njtcPortal');
    if (portal) portal.style.display = '';

    // Set header name
    const phName = document.getElementById('phName');
    if (phName) phName.textContent = user.firstName || user.name || 'Staff';

    // Set header role + school
    const phRole = document.getElementById('phRole');
    if (phRole) {
        const primarySchool = (user.assignments && user.assignments.length > 0 && user.assignments[0].schools && user.assignments[0].schools.length > 0)
            ? user.assignments[0].schools[0]
            : null;
        phRole.textContent = (user.role || 'Staff') + (primarySchool ? ' · ' + primarySchool : '');
    }

    // Determine mapped role for resources tab
    const roleMap = {
        'Tutor': 'tutor',
        'Substitute': 'tutor',
        'Certified Tutor': 'tutor',
        'Dual Role': 'tutor',
        'Instructional Coach': 'coach',
        'Site Coordinator': 'site-lead'
    };
    const mappedRole = roleMap[user.role] || 'tutor';

    // Load role-specific resources content
    loadRoleContent(mappedRole);
...
function updateTime() {
    ...
    if (phTime) phTime.innerHTML = dateString + '<br>' + timeString;
}

// ── Inline styles for sample posts (Knowtion guide) ──────────────────────────

function injectSamplePostStyles() {
    if (document.getElementById('njtc-sample-posts-style')) return;
    const style = document.createElement('style');
    style.id = 'njtc-sample-posts-style';
    style.textContent = `
    .sample-posts { display: grid; gap: 1.25rem; margin-top: 1rem; }
    .sample-post { background: linear-gradient(135deg, rgba(232,62,140,0.08), rgba(253,126,20,0.05)); border-left: 4px solid #e83e8c; border-radius: 12px; padding: 1.25rem; transition: all 0.3s ease; }
    .sample-post:hover { transform: translateX(8px); box-shadow: 0 4px 16px rgba(232,62,140,0.15); }
    .sample-post strong { display: block; font-family: 'Epilogue', sans-serif; font-size: 0.875rem; font-weight: 700; color: #e83e8c; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem; }
    .sample-post p { font-size: 1rem; line-height: 1.7; color: rgba(255,255,255,0.8); margin: 0; font-style: italic; }
    `;
